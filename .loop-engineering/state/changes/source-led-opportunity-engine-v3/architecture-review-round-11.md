...
Supabase REST is only a transport layer. The claim RPC restores the frozen authority
hash, the doctor reads private producer state through one bounded security-definer RPC,
and `service_role` receives no run/job table SELECT. Migration order is
base→V3.12→V3.13→V3.14→V3.15, additive and apply-twice safe. The V3.14 identity upgrade
now accepts both reviewed historical predecessors and converges them to one canonical
identity; mixed or duplicate identities still fail closed. This closes the actual
production upgrade path without introducing a side-channel migration. Runtime rollback stops the
scheduler and restores the previous manifest while leaving additive objects inert.

The authority reader upgrade changes only the closed per-connector conservation
ceiling from 1,000 to 2,000. It retains deterministic latest-family ordering,
200-row immutable pages, complete frozen-revision lineage and the existing 12,000-job
terminal bound. It accepts the observed 3,385-revision production corpus without
...
The final production transport closure moves only the mention stage-barrier claim
into a second private security-definer helper because the predecessor applies its
3 MiB bound before the outer wrapper can project candidates. The helper duplicates
the predecessor's exact run, owner-token, lease, row-lock and attempt transition for
that one job kind, then delegates candidate assembly to the already bounded reader.
It is owned by the legacy correctness role; the REST claim owner receives EXECUTE
only, and service_role retains access solely to the outer authority-carrying RPC.
Every non-barrier claim still traverses the authoritative predecessor unchanged.

The candidate-funnel transport now emits its hash-bound `candidate_funnel_input`
as the sole copy of the mention result. The redundant predecessor composite fields
are nulled only on the transient claim after the read bundle has been constructed;
...
Round 38 reviews the transient chunk transport closure. Retry authority is confined
to the private official-ingestion append/apply RPC whose immutable application ledger
already makes a repeated call idempotent. Only network transport exceptions, HTTP 429
and HTTP 5xx cross the retry boundary, with at most three attempts and bounded backoff;
HTTP 4xx database authority, hash, lease and integrity rejections never retry. The
worker cannot skip a member because terminal completion still requires the exact
ledger identity for every staged non-terminal chunk, and it cannot duplicate a fact
because both the ledger and underlying append authorities converge on the same
immutable identity. No claim, completion, public API, ranking, decision, scheduler,
database role or Promotion authority changes. Architecture result: `PASS`,
`P0=0 P1=0 P2=0`.

Round 39 reviews the gateway-safe transaction repair. It changes only the internal
partition of already-authorized append/apply requests: financial, price,
corporate-action and valuation datasets use at most five rows per REST transaction,
while their aggregate immutable manifest, item hashes, source cutoff, producer
identity and terminal conservation remain identical. Calendar ingestion keeps its
200-row bound. Failure metadata crosses only an allowlisted internal path and ordinal;
response bodies, SQL, URLs and credentials remain outside the diagnostic plane.
DecisionEnvelope, ranking weights, public reads, database roles, scheduler ownership,
LINE/dispatch and Promotion are unchanged. Architecture result: `PASS`,
`P0=0 P1=0 P2=0`.

Round 40 reviews the valuation-metric normalization boundary found by the first
gateway-safe production run. Normalization occurs inside the private official
ingestion stream before canonical chunk hashing: each PE/PB metric independently
converges to the same V3.13 authority range, so an invalid PE cannot invalidate a
valid PB or four adjacent observations. Rows with neither valid metric do not enter
the immutable valuation authority. This does not loosen the valuation model, create
an action, change ranking weights, change public schemas, broaden database grants or
alter the producer DAG; it makes the transport and append contracts identical.
Architecture result: `PASS`, `P0=0 P1=0 P2=0`.

Round 41 reviews the durable process-resume authority boundary. Lease acquisition
already authenticates and binds one adapter process to the exact run authority;
retaining that identity for the private completion wrapper does not grant registry
read authority or allow page suppression. The materialized-page cache remains empty
until a claim returns non-empty pages whose embedded hash matches the run. Completion
still revalidates the hash against the run inside PostgreSQL and delegates unchanged
lease, result-hash, chunk-conservation and successor semantics to the predecessor.
This closes the process-boundary gap without changing the producer DAG, provider
authority, ranking, DecisionEnvelope, public mutation, scheduler owner, database
grants, LINE/dispatch or Promotion. Architecture result: `PASS`,
`P0=0 P1=0 P2=0`.

Round 43 reviews the analysis durable-transport and status-oracle closure. The
immutable fact plane continues to exist in the facts predecessor and exactly once in
each decision persistence payload. Projection decisions carry only revision identity
and evaluation metadata; compact projection receives only the calendar authority hash
plus bounded coverage summary that it consumes. No decision, source candidate,
citation or evidence is filtered, and the production-shaped result remains below the
