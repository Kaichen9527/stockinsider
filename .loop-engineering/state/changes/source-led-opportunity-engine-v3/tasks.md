...
## V3.17 single release closure — only active action queue
...
```mermaid
flowchart LR
  S["Curated/community sources"] --> U["Discovery union"]
  O["Official events"] --> U
  A["Market anomalies"] --> U
  U --> I["Verified change inbox"]
  I --> E["Evidence, contradictions, invalidation"]
  E --> F["Save / dismiss / follow-up feedback"]
  E --> R["Shadow ranking"]
  F --> L["Point-in-time learning corpus"]
  R --> L
  L --> G{"Promotion gates"}
  G -->|pass| P["Optional homepage ordering"]
  G -->|fail / immature| I
  MR["model_runner_v3"] -. "independent, influence:none" .-> L
```
...
sequenceDiagram
  ...
  participant UI as Shadow UI
  participant E as Evaluator
  C->>DB: append exact revision or typed failure
  DB->>W: one bounded leased payload
  W->>DB: atomic result + successor
  DB->>UI: last complete public projection
  UI->>DB: save/dismiss/follow-up feedback
  DB->>E: cutoff-frozen inputs and outcomes
  E->>DB: non-promoting evaluation evidence
```
...
```mermaid
flowchart TD
  A["Source + authority appends"] --> B["Immutable observations / manifests"]
  B --> C["Database-owned begin + exact identities"]
  C --> D["Deterministic successor DAG"]
  D --> E["Typed one-job worker executors"]
  E --> F["Atomic native rows + successor"]
  F --> G["Finalization invariants"]
  G --> H["Same-run public/detail projection"]
  H --> I["Shadow UI"]
  G --> J["Outcomes + shadow evaluation"]
  J --> K["Elapsed/authority promotion gates"]
  MR["model_runner_v3"] --> AR["Signed assistive artifact registration"]
  AR -. "influence:none" .-> H
```
...
```mermaid
flowchart LR
  U["Pure unit/property"] --> DB["Fresh PostgreSQL migration/catalog"]
  DB --> SEC["RLS/grant/principal/RPC matrix"]
  SEC --> DAG["All-mode DAG + crash/retry"]
  DAG --> INT["Source-to-projection integration"]
  INT --> API["Exact route/public bytes"]
  API --> UI["Browser/accessibility"]
  INT --> PERF["EXPLAIN + envelope limits"]
  RUN["Independent pinned-host runner"] --> RG["Runner verification gate"]
```
