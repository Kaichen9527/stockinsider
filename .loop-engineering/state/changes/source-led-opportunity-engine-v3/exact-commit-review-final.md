...
- Final reviewed repair/tree: `e85437c2d02081108af3fbf035743976eb659336` / `75edd0a15536c19d57e7db3f87dafc83cf4fee6b`
- Full final range: `5186543fce7dd7a89ca5f6f2907162b7a0412c12..e85437c2d02081108af3fbf035743976eb659336`
- Active graph: `722095adecd208b54cc794f72e262dd5b77ed970da80f0ab5771a55259651625`
- Scope: the 68-path product and deployment-admission diff, including the additive SQL migration, official feed loader, authenticated resumable authority acquisition, shared candidate roster, research calculation, page integration and publication conservation checks.
...
The V3.18 authority digest repair and graph-bound Requirements/Architecture reviews were merged to protected main separately in PR #281 and are carried into this subject unchanged. This final product diff does not modify the protected graph, exact-review grammar or production publication gate. Its active graph identity reflects the already merged V3.18 metadata repair. The final deployment-admission patch resolves the physical script path before deciding whether to run the CLI; it prevents the `current` symlink used by systemd and the heavy-operation wrapper from silently skipping the disk/memory check. A child-process regression proves a symlink invocation with an invalid budget exits blocked with structured evidence.
...
- The capacity CLI symlink regression passed 3/3 resource-check tests. TypeScript, lint and production web build passed. The focused PostgreSQL migration/RLS/idempotence checks and seven browser checks passed before this final protected metadata merge; the final admission patch changes no product UI or migration bytes.
