...
ordinaryTest('operation and resource identities are deterministic and bound', () => {
  ...
  assert.deepEqual(MODEL_RUNNER_IDENTITY.find(([name]) => name === 'codexVersion'), ['codexVersion', '0.154.0-alpha.6.2']);
  assert.deepEqual(MODEL_RUNNER_IDENTITY.find(([name]) => name === 'contractVersion'), ['contractVersion', 'model-runner-v3.6']);
  assert.deepEqual(MODEL_RUNNER_IDENTITY.find(([name]) => name === 'hostPinVersion'), ['hostPinVersion', 'model-runner-host-pins-v3.17']);
...
ordinaryTest('host pin fixture has an exact hash-bound format', async () => {
  ...
  assert.equal(pins.fixtureVersion, 'model-runner-host-pins-v3.17');
  assert.equal(pins.executables.find((entry) => entry.name === 'codex').version, 'codex-cli 0.154.0-alpha.6.2');
...
ordinaryTest('version probes admit only closed known sandbox diagnostics', () => {
  ...
  assert.equal(validatedVersionOutput(codex, 'codex-cli 0.154.0-alpha.6.2\n', aliasWarning),
...
ordinaryTest('disabled doctor accepts only the protected v3.9 compatibility selector for the exact v3.17 fixture', () => {
