...
const relativePath = 'migrations/20260911_retention_archive_v1.sql';
...
async function applyMigration(plan, sourceCommit) {
...
const verification = options.apply ? await applyMigration(plan, options.sourceCommit) : null;
process.stdout.write(`${JSON.stringify({ protocol: 'stockinsider-retention-archive-v1-migration-plan',
  sourceCommit: options.sourceCommit || null, migration: { relativePath: plan.relativePath, bytes: plan.bytes, sha256: plan.sha256 },
