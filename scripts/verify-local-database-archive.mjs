...
try {
  ...
  const header = Buffer.alloc(55), tag = Buffer.alloc(16);
  await file.read(header, 0, header.length, 0);
  await file.read(tag, 0, tag.length, info.size - tag.length);
  const decipher = createDecipheriv('aes-256-gcm', key, header.subarray(11, 23));
  ...
  await pipeline(file.createReadStream({ start: 55, end: info.size - 17, autoClose: false }), decipher, child.stdin);
  ...
  console.error('database_archive_verification_failed'); process.exitCode = 1;
} finally {
