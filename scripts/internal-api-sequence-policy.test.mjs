...
test('hourly research resume ignores symbol-scoped canary receipts', () => {
  ...
  assert.match(route, /select\('id,technical_session_date,pipeline_run_id,status,failed_count'\)/u);
  assert.match(route, /completedCandidateResearchSessions\(/u);
  assert.match(route, /from\('pipeline_runs'\)\.select\('id,status'\)/u);
