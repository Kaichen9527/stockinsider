...
function deriveDiscoveryDisposition({ linked, seedSymbols, priorLedger, evidenceHash, sourceAvailable = true, confidence = 1 }) {
  ...
  if (prior?.materialEvidenceHash === evidenceHash) return { disposition: 'unchanged', reason: 'same_material_evidence',
    researchDisposition: 'not_selected', researchReason: null, seedMembership: inSeed ? 'in_seed' : 'out_of_seed' };
  return {
