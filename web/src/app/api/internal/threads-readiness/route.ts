...
import { sourceExecutionPolicy, threadsProfileMonitoringPolicy } from '@/lib/source-policy';
import { evaluateThreadsReadiness } from '@/lib/threads-discovery';
...
export async function GET(request:Request) {
  ...
  const readiness=evaluateThreadsReadiness(metadata,credential.data?.status||null);
  const profileMonitoring=threadsProfileMonitoringPolicy(metadata.granted_scopes);
  const expired=readiness.reason==='threads_token_expired';
  const canary=readiness.publicSearchVerified;
  return NextResponse.json({ok:true,readiness:{
    ...
    permissions:{status:canary?'public_search_verified':'unverified',reason:readiness.reason},
    publicSearch:{status:canary?'verified':'canary_pending',zeroRowStreak:readiness.zeroRowStreak},
    profileMonitoring:{status:profileMonitoring.disposition,reason:profileMonitoring.terminalReason},
