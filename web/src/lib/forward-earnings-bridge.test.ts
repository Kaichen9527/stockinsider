...
function fact(factKey: string, year: number, quarter: number, value: number): ReportedFinancialFact {
  const end = [`${year}-03-31`, `${year}-06-30`, `${year}-09-30`, `${year}-12-31`][quarter - 1];
  return { factId: `${factKey}-${end}`, factKey, periodStart: `${year}-01-01`, periodEnd: end, value, unit: 'TWD', sourceRef: `mops:${end}` };
}
...
test('forward bridge is explicit, reproducible, and separates assumptions from reported facts', () => {
  ...
  const metrics = {
    ...
    quarterly_diluted_eps: [1.6, 3.4, 5.4, 7.6, 2, 4.2, 6.6, 9.2],
  };
...
test('missing eight-quarter bridge inputs fail closed', () => {
  assert.deepEqual(buildForwardEarningsBridge([fact('quarterly_revenue', 2025, 1, 100)]), {
    status: 'insufficient',
    missing: [
      'quarterly_revenue_8_discrete_quarters', 'quarterly_gross_profit_8_discrete_quarters',
      'quarterly_operating_income_8_discrete_quarters', 'quarterly_net_income_attributable_to_common_8_discrete_quarters',
      'quarterly_diluted_eps_8_discrete_quarters',
    ],
  });
});
