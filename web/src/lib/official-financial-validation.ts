...
export function validateOfficialFinancialFact(
  ...
  const values = new Map(periodPeers.filter((p) => finite(p.value)).map((p) => [String(p.fact_key), Number(p.value)]));
  const identities = [
  ...
  const inputHash = createHash('sha256').update(JSON.stringify({ fact, peers: periodPeers, provenance, version: OFFICIAL_FINANCIAL_VALIDATOR_VERSION })).digest('hex');
