...
export const AUO_RESEARCH_VERSION = 'auo-2409-2026-09-23.v3';
...
export const auoArticleSections = [
  {
    id: 'question', number: '01', title: '核心投資問題：34.70 元仍在交易轉型選擇權，證據沒有增加',
    paragraphs: [
      '友達現在的市場敘事有三層。第一層是面板循環；第二層是 BHTC 車用系統與垂直場域；第三層是 Micro LED CPO、玻璃核心基板與 Intel 合作傳聞帶來的技術定位重估。36.65 元是市場在 9/22 真實成交形成的高點，9/23 則以 34.70 元收盤；兩者都不是假的共識，也都不是經公司確認的基本面合理價。研究的責任是拆出價格需要哪些營運與商業化條件，並在價格回落時檢查證據是否同步改變。',
      ...
      '傳統營運的 2027 樂觀情境約 EPS 1.37 元、24 倍約 32.9 元，仍低於最新 34.70 元。約 1.8 元的差額可解讀成市場替轉型選擇權、資產重估與資金動能付費。9/23 的 5.32%回檔縮小了溢價，卻沒有增加具名客戶、驗證節點、試產、產能、良率或毛利證據，因此中期結論仍是等待驗證。',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '本地與資產面比較則應看群創與友達自身的歷史 P/B。兩者都受面板循環與重資產結構影響；友達可以因非顯示業務得到溢價，但溢價應由 ROE、自由現金流與獲利穩定性驗證。34.70 元相對 20.46 元帳面價值約為 1.70 倍 P/B，仍高於這份 60 個月月底樣本的 1.65 倍最高值；這代表市場仍在支付歷史資產框架之外的轉型期待，而不是證明該期待必然兑现。',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '34.70 元若給 20 倍，需要 EPS 1.74 元；若給 24 倍，需要 1.45 元。這比模型的 2027 基本 EPS 0.62 元高，也略高於樂觀 EPS 1.37 元。若市場其實交易 2029 年成果，則必須把更遠期 EPS 折現，並在客戶、試產、產能與毛利資料出現後重建模型。本版先呈現敏感度，不替尚未確認的 Intel 合作填收入或成功率。',
      '因此本版沒有把 34.70 元硬改成新的基本目標價。傳統營運的情境值、P/B 資產交叉檢查與轉型敏感度分開呈現。市場價格可領先財報，但若後續只有轉載而沒有客戶驗證，選擇權會收縮；若出現 design win、量產規格與可核對的毛利，才有理由把轉型移入財務預估。',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '9/23 開盤與最高均為 36.65 元，之後回落至 34.70 元，單日跌 5.32%、成交量 9.49 億股。收盤仍在 32.2 元原突破區之上，但這只是目標到達後的第一個完整回檔日，高成交量也不足以稱為量縮整理完成。原突破劇本的預期報酬已實現，不能把 36.6 元目標再改高來合理化追價。',
      '新部位等待兩種條件：價格整理後量縮守住突破區並重新轉強，或公司／供應鏈提出足以重建盈利的證據。若只剩傳聞擴散、量價背離或收盤跌回原突破區，事件交易的風險快速升高。短線強勢與中期估值可以同時成立，前者描述資金與預期，後者要求獲利兑现。',
      '頁面會保留原劇本的發布日、觸發日、確認日與目標到達日，不再每日重算門檻後永遠顯示「等待」。9/23 的回檔只更新後續觀察，不回寫已完成的劇本。',
    ], sources: ['S14'],
  },
  {
