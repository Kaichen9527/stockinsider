...
test('17-symbol valuation routing is fail closed', () => {
  ...
  assert.deepEqual(candidateValuationPolicy({ symbol: '2002', multipleMonthsCovered: 60, next12mBridgeComplete: false, verifiedTurnaroundPath: false }), { basis: 'normalized_cycle', canPublishTarget: false, reason: 'normalized_cycle_earnings_and_asset_return_inputs_incomplete' });
