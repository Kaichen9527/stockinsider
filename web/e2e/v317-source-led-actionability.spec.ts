...
test('V3.17 keeps a support setup in the waiting lane, makes its snapshot navigable, and keeps CTA contrast readable',async({page})=>{
  ...
  for(const colorScheme of ['light','dark'] as const){
    ...
    await expect(page.getByTestId('research-snapshot-detail')).toContainText('at_support');
    await expect(page.getByTestId('research-gate-waterfall')).toContainText('technical · pass');
  }
